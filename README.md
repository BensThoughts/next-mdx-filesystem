...
getSlugs returns `IStaticPath[]`. It can be used directly and easily in
...
Given that there is no way to know if a slug points to a directory or an mdx
file prior to calling the function it returns `isDirectory: boolean` as a
property of the returned object. If the route that the slug points to is a
...
If the route was a directory the `directory` property will contain one of 2 data
structures depending on the `returnType` option.
